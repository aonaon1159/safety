...
<?php
    ...
    WHERE status = "active" AND del = "false" AND name_category = 4
    ORDER BY sequence');
    $stmt->execute();
    $training_public = $stmt->fetchAll(PDO::FETCH_ASSOC);

    //ดึงข้อมูล group_training_public
    $stmt = $safetyDb->prepare('SELECT id_group_training_public,name_group FROM group_training_public WHERE name_category = 4');
    $stmt->execute();
